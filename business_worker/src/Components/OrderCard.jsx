...
function OrderCard({ order }) {
	...
	useEffect(() => {
		if (menu.length > 0) {
			let filteredMenu = []
			order.cart.map(cart => menu.map(meunItem => meunItem.id === cart.id && filteredMenu.push({...meunItem, quantity: cart.quantity})))
	...
	return (
		...
		<Order onMouseUp={() => navigate(`/queue/${order.id}`)}>
			...
			<Content>
				...
				<a target='_blank' href={`https://www.google.com/maps?q=${order.location.positions[order.location.positions.selected][0]},${order.location.positions[order.location.positions.selected][1]}`}>Location: Maps</a>
