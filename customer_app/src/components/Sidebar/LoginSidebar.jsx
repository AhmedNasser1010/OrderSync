...
const LoginSidebar = () => {
  ...
  return (
    <>
      ...
      <div className={`login-sidebar fixed top-0 ${direction === 'rtl' ? 'left-0' : 'right-0' } h-full overflow-y-scroll justify-between bg-white transition-all duration-500 z-20 sm:px-20 px-5 py-5 w-full sm:py-10 flex flex-col sm:w-[500px] ${isLoginSidebarOpen ? (direction === 'rtl' ? "-translate-x-0" : "translate-x-0") : (direction === 'rtl' ? "-translate-x-full" : "translate-x-full")}`}>
        <div>
              ...
              !user?.userInfo ?
                <>
                  <div className='flex items-center justify-between mb-5'>
                    <div>
                      <h2 className='text-color-1 text-3xl font-ProximaNovaMed'>{ status === 'SIGNUP' ? t('Signup') : t('Login') }</h2>
                      ...
                      <p className='font-ProximaNovaThin mt-1'>{t('and')} <span className='text-color-2 font-ProximaNovaMed'>{t('Enjoy your time')}</span></p>
                    </div>
                    <div>
                ...
                <>
                  ...
                  <button onClick={() => setExpandUserInfo(expandUserInfo => !expandUserInfo)} className={`settings-btn relative w-full py-4 uppercase text-base text-white font-ProximaNovaSemiBold cursor-pointer mt-5 mb-5 ${user?.userInfo?.name && user?.userInfo?.phone && user?.userInfo?.secondPhone ? 'bg-color-11' : 'bg-red-500'}`}><span className='text-xl absolute left-[20px] top-[50%] -translate-y-1/2'>{ user?.userInfo?.name && user?.userInfo?.phone && user?.userInfo?.secondPhone ? <IoIosCheckmarkCircle /> : <IoIosCloseCircle /> }</span> {t('Update User Information')}</button>
        ...
        { user?.userInfo &&  <button onClick={handleLogout} className='w-full py-4 uppercase text-base text-red-500 font-ProximaNovaSemiBold cursor-pointer mt-10 border border-red-500'>{t('Logout')}</button>}
      </div>
      ...
      {/*<div className={`fixed z-20 top-0 left-0 ${direction === 'rtl' ? 'left-[-100%]' : 'left-[150%]' } bottom-0 bg-red-500 w-full transition-all duration-1000`}></div>*/}
