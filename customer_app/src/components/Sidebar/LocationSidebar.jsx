...
const LocationSidebar = () => {
  ...
  const handleSearchLocation = async (e) => {
    console.log("handleSearchLocation: ", SearchText)
    // try {
  ...
  const handleUserLocation = async (placeid) => {
    console.log("handleUserLocation: ", placeid)
    // try {
  ...
  return (
    <>
      <div
        className={`location-sidebar fixed top-0 ${direction === 'rtl' ? 'left-0' : 'right-0' } h-full overflow-y-scroll bg-white transition-all duration-500 z-20 sm:px-20 px-5 py-5 w-full sm:py-10 flex flex-col sm:w-[500px] ${isLocationSidebarOpen ? (direction === 'rtl' ? "-translate-x-0" : "translate-x-0") : (direction === 'rtl' ? "-translate-x-full" : "translate-x-full")}`}
      >
        ...
        <div className="relative">
          <div className="relative">
            <input
              ...
              placeholder={t("Search for area, street name..")}
