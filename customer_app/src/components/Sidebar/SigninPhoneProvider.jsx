  ...
  useEffect(() => {
  	verifyOtp(formData.otp).then(user => {
  		...
  		if (user?.uid) {
  			if (status === 'SIGNUP') {
  				if (isLoggedInBefore === false) {
    				DB_ADD_DOC('customers', user.uid, customerSchema({ user: user?.uid, name: formData?.name, phone: formData?.phoneNumber, referredBy: formData?.referral, provider: 'Phone', partnerUid: user?.partnerUid }))
