...
/**
 * Reverses cash back on an order cancellation/return. Claws back the cashback
 * credit earned from the order and restores any credits the order redeemed,
 * back to the customer's wallet with their original expiry dates.
 *
 * Idempotent: trusts the order to have terminal cancellation state and only
 * mutates credits that are still in the relevant active/redeemed state.
 */
export async function handleOrderClawback(
  ...
  try {
    ...
    // Redeemed credits (those the order consumed).
