		...
		<>
		{
			(topChains && topChains?.length != 0) &&
				<section id='top-chain' className='relative'>
					<h2 className='font-GrotBlack text-2xl pb-5 pt-5'>{t("Top restaurant chains in El-Ayat")}</h2>
					<div className={`scroll-buttons absolute top-0 flex gap-2 ${direction === 'rtl' ? 'left-10': 'right-10'}`}>
						<button onClick={handleTopChainLeft} className={`scroll-left text-white flex justify-center cursor-pointer ${direction === 'rtl' && 'order-1'}`}>
						...
						<div className='flex gap-8'>
							{
								topChains?.map((res) => (
								  <Link className='relative transition-all hover:scale-95' key={res?.accessToken} to={`/restaurants/${res?.accessToken}`}>
								    {
								      // res?.accessToken?.aggregatedDiscountInfoV3 ? <RestaurantCardwithOffer info={res?.info} /> : <RestaurantCard info={res?.info} />
								      <RestaurantCard info={{
											    areaName: 'El-Ayat',
