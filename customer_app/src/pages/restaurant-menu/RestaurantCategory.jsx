        ...
        <>
          <div className='accordion-body'>
            {
              filteredMenuItems?.map(item => {
                const { finalPrice, isAvailableForUser } = item?.discount?.code ? priceAfterDiscount(item.price, item.discount, user, ResInfoData.id) : { finalPrice: item.price, isAvailableForUser: false }
                ...
                return (
                  <div key={item?.id} className='item flex items-start justify-between pb-8'>
                    <div className='md:w-auto w-3/5'>
                      ...
                      <h4 className={`text-base ${discountIncluded ? 'text-color-2' : 'text-color-9'} font-ProximaNovaMed`}>{item?.title}</h4>
                      {
...

          {
            ...
            <Popup
              ...
              description={t('Your cart contains items from other restaurant. Would you like to reset your cart for adding items from this restaurant?')}
