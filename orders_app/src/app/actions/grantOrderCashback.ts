...
/**
 * Grants cash back on a delivered order. Called by the kitchen app when an
 * order is marked complete directly (READY -> DELIVERED via the
 * `allowMarkComplete` setting, no driver assigned). Idempotent: if the order
 * already carried cashback, it is not granted twice. Runs under the Admin SDK
 * so it can write the wallet_credits ledger (which clients cannot).
 */
