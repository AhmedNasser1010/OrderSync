...
function TopRestaurant() {
  ...
  const topChains = useMemo(() => {
    return restaurants.filter(
      (restaurant: RestaurantDocument) => restaurant?.topChains
    );
  }, [restaurants]);

  const handleTopChainLeft = () => {
    const topCategory = document.querySelector(".top-chain-category");
    topCategory && (topCategory.scrollLeft = topCategory.scrollLeft - 250);
  };
      ...
      {topChains && topChains?.length != 0 && (
        <section id="top-chain" className="relative">
          <h2 className="font-GrotBlack text-2xl pb-5 pt-5">
            {t("Top restaurant chains in El-Ayat")}
          </h2>
          <div
            className={cn(
          ...
          <div className="top-chain-category overflow-x-scroll scroll-smooth scrollbar-hide max-w-[1500px]">
            <div className="flex gap-8">
              {topChains?.map((res: RestaurantDocument) => (
